...
# ─── [FLOW V2] Tải clip video ĐÃ gen sẵn về máy (CDN cookie-gated → SW fetch) ───
@app.get("/v2/media_urls")
async def _media_urls():
    """Liệt kê URL media đang giữ (image/video) — chẩn/điều phối tải."""
    b = await get_active_bridge()
    urls = dict(getattr(b, "media_urls", {}))
    vids = {k: v for k, v in urls.items() if "/video/" in (v or "")}
...
async def _play_tile():
    ...
    b = await get_active_bridge()
    res = await b.ui_generate({"prompt": "__PLAYTILE__", "timeoutMs": 20000})
    return (res.get("result") if isinstance(res, dict) else None) or res


@app.post("/v2/fetch_video/{media_id}")
async def _fetch_video(media_id: str):
    """Tải bytes video theo media_id: echo URL /video/ cho SW tải (cookie) → media_blobs → lưu mp4."""
    import time as _t, uuid as _u, os as _o
    from omniflash.generators.common import download_video
    b = await get_active_bridge()
    url = getattr(b, "media_urls", {}).get(media_id.lower())
    if not url:
        raise HTTPException(status_code=404, detail=f"chưa có URL /video/ cho {media_id} (gọi /v2/play_tile trước)")
    await b.ui_generate({"prompt": "__FETCHBYTES__ " + url, "mediaId": media_id, "timeoutMs": 120000})
    fn = f"flow_vid_{int(_t.time())}_{_u.uuid4().hex[:6]}.mp4"
    out = _o.path.join(OUTPUT_DIR, fn)
    if not await download_video(b, media_id, out):
        raise HTTPException(status_code=500, detail=f"tải bytes hỏng (blob={media_id.lower() in getattr(b,'media_blobs',{})})")
    return {"file": out, "media_id": media_id, "bytes": _o.path.getsize(out)}
...
class VideoGenerationRequest(BaseModel):
    ...
    # [WEVIDEO patch] FL — first+last frame control (start + end image)
    end_image_base64: Optional[str] = Field(None, description="Optional base64 END image for first+last frame (FL)")
...
async def openai_generate_image(req: ImageGenerationRequest):
    ...
    # [FLOW V2] flow.google.com dùng cookie auth → API aisandbox (generate_image) chết.
    ...
    try:
        ...
        first_error = None
        if ui_img_mode:
            from omniflash.generators.ui import generate_image_ui
            for chunk_size in chunks:
                try:
                    ...
                    first_error = e
                    log.error(f"Error in generate_image_ui chunk: {e}")
        else:
            tasks = []
            for chunk_size in chunks:
                tasks.append(
    ...
    for i, r in enumerate(results):
        ...
        # [FLOW V2] UI transport: CDN cookie-gated → urllib 403. Ưu tiên bytes base64 do SW tải sẵn.
        download_success = False
        _blob = getattr(active_bridge, "media_blobs", {}).get(str(r.get("media_id") or "").lower())
        if _blob:
            try:
                with open(out_path, "wb") as _f:
                    _f.write(base64.b64decode(_blob))
                download_success = os.path.getsize(out_path) > 0
                if download_success:
                    log.info("Saved image: %s [SW base64]", out_path)
            except Exception as _e:
                log.warning("Giải base64 ảnh hỏng: %s", _e)
                download_success = False
...
async def openai_generate_video(req: VideoGenerationRequest):
    ...
    from omniflash.config import FLOW_TRANSPORT
    if FLOW_TRANSPORT == "ui":
        # [FLOW V2 11/09] không gọi /v1/credits aisandbox (401 — hết Bearer); dùng số credit
        # quan sát được từ lần gen trước (có thể stale). None → bỏ gate, chỉ cảnh báo.
        balance = getattr(active_bridge, "last_credits", None)
        if balance is None:
            log.warning("UI transport: chưa quan sát được credits — bỏ credit gate lần này.")
    else:
        try:
    ...
    # [WEVIDEO patch] upload END image for FL if provided as base64
    end_media_id = req.end_media_id
    if req.end_image_base64 and not end_media_id:
        b64e = req.end_image_base64
        if "," in b64e:
            b64e = b64e.split(",")[1]
        tmp_end = os.path.join(OUTPUT_DIR, f"fl_end_{int(time.time())}_{uuid.uuid4().hex[:6]}.png")
        try:
            with open(tmp_end, "wb") as f:
                f.write(base64.b64decode(b64e))
            from omniflash.generators.i2v import upload_image as _upload_end
            end_media_id = await _upload_end(active_bridge, tmp_end, project_id)
            if not end_media_id:
                raise HTTPException(status_code=500, detail="Failed to upload END image to Google Flow.")
        except Exception as e:
            log.exception("Error uploading END image")
            raise HTTPException(status_code=500, detail=f"END image upload error: {str(e)}")
        finally:
            if os.path.exists(tmp_end):
                try:
                    os.remove(tmp_end)
                except Exception:
                    pass
    ...
    if ui_mode and req.dry_run:
        ...
        try:
            info = await dry_run_ui(active_bridge, req.prompt, aspect_key, req.duration)
        except Exception as e:
            raise HTTPException(status_code=400, detail=str(e))
    ...
    try:
        ...
        if ui_mode:
            ...
            # [WEVIDEO patch] FL — first+last frame: chuyển mượt từ ảnh đầu sang ảnh cuối
            from omniflash.generators.i2v import generate_video_fl
            media_ids = await generate_video_fl(active_bridge, req.prompt, aspect_key, project_id, image_media_id, end_media_id, duration=req.duration)
    ...
    async def poll_and_download(media_id: str, index: int):
        # [FLOW V2 11/09] UI transport: driver đã chờ render xong + background.js đã tải bytes vào
        # bridge.media_blobs → bỏ poll_status (đường aisandbox 401).
        if ui_mode or str(media_id).lower() in getattr(active_bridge, "media_blobs", {}) \
                or str(media_id).lower() in getattr(active_bridge, "media_urls", {}):
            success = True
        else:
...
async def get_flow_credits():
    ...
    from omniflash.config import FLOW_TRANSPORT
    if FLOW_TRANSPORT == "ui":
        # [FLOW V2 11/09] không còn API credits; trả giá trị quan sát từ UI (YhhmEf/nzlxg)
        if getattr(active_bridge, "last_credits", None) is None:
            raise HTTPException(status_code=503, detail=(
                "credits chưa quan sát được (UI transport) — gen 1 lần hoặc gọi dry_run rồi thử lại"))
        return {"credits": active_bridge.last_credits, "source": "ui-observed",
                "observed_at": active_bridge.last_credits_at, "transport": "ui"}
    try:
...
async def health():
    ...
    from omniflash.config import FLOW_TRANSPORT
    return {
        "transport": FLOW_TRANSPORT,  # [FLOW V2 11/09]
        ...
        # [VÁ RIÊNG 11/09] số URL media ký sẵn đang giữ — để kiểm tra đường tải video
        "media_urls": len(getattr(bridge, "media_urls", {})),
    }
...
# ─────────────────────────────────────────────────────────────────────────
# [TẠM 18/08 — chẩn đoán tải video, GỠ SAU] Gọi tuỳ ý một endpoint Flow với
# media đã render sẵn → KHÔNG tốn credit, thử bao nhiêu lần cũng được.
# ─────────────────────────────────────────────────────────────────────────
@app.post("/debug/probe")
async def _debug_probe(req: Request):
    """body: {"path": "/v1/...", "method": "GET|POST", "body": {...}}"""
    if bridge is None:
        return {"error": "bridge chưa sẵn sàng"}
    p = await req.json()
    try:
        res = await bridge.api_request(
            p["path"], p.get("body", {}),
            captcha_action=p.get("captcha_action", ""),
