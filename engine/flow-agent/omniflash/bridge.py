...
from .config import (
    ...
    MAX_CONCURRENT_REQUESTS, REQUEST_MIN_INTERVAL, FLOW_TRANSPORT,
)
...
class ExtensionBridge:
    ...
    def __init__(self):
        ...
        # [VÁ RIÊNG 18/08] URL ký sẵn của media, do extension trích từ response
        # tRPC của trang Flow và đẩy sang bằng message `media_urls_refresh`.
        # Bản upstream 16/07 KHÔNG nhận message này nên video tải bằng base64
        # qua /v1/media/{id} — đường đó nay Google trả 400 INVALID_ARGUMENT.
        self.media_urls: dict[str, str] = {}
        # [FLOW V2 11/09] credits quan sát được qua UI transport (response YhhmEf/nzlxg)
        # — không còn gọi /v1/credits của aisandbox (401 vì hết Bearer).
        self.last_credits = None
        self.last_credits_at = None
        # [FLOW V2 11/09] bytes video (base64) do background.js tải về trong SW — CDN mới cookie-gated
        # nên Python urllib 403; download_video ưu tiên đọc từ đây. Keyed by mediaId (submit).
        self.media_blobs: dict[str, str] = {}
    ...
    async def wait_for_extension(self, timeout=90, max_retries=3):
        ...
        # [FLOW V2] UI transport (flow.google.com) dùng COOKIE — KHÔNG cần Bearer token.
        # Bỏ auto-fix (tránh reload tab liên tục phá gen đang chạy). Chỉ WS nối là đủ.
        try:
            from .config import FLOW_TRANSPORT
            if FLOW_TRANSPORT == "ui":
                log.info("UI transport: bỏ qua auto-fix token (v2 dùng cookie).")
                return True
        except Exception:
            pass
    ...
    async def health_check(self):
        """Quick check extension sẵn sàng. [FLOW V2] UI transport (flow.google.com) KHÔNG cần
        flow_key — Bearer ya29 chỉ bắt được trên labs.google/aisandbox cũ; v2 dùng cookie.
        Chỉ cần WS nối + extension trả lời get_status là đủ để lái UI."""
        if not self._ws:
            return False
        ui = FLOW_TRANSPORT == "ui"
        if not ui and not self._flow_key:
            return False
        try:
    ...
    async def _handle_message(self, data):
        ...
        if msg_type == "token_captured":
            ...
            log.info("Extension ready (flowKey=%s, bg=%s)", "yes" if data.get("flowKeyPresent") else "no", data.get("bgVersion", "?"))
            if data.get("flowKeyPresent") and self._flow_key:
                self._connected.set()

        elif msg_type == "media_urls_refresh":
            # [VÁ RIÊNG 18/08] Chỉ có URL này khi trang Flow đang mở và gọi tRPC.
            entries = data.get("urls") or []
            for e in entries:
                mid, url = e.get("mediaId"), e.get("url")
                if mid and url:
                    self.media_urls[mid] = url
            if entries:
                log.info("Nhận %d URL media ký sẵn (tổng đang giữ: %d)",
                         len(entries), len(self.media_urls))
    ...
    async def _do_api_request(self, url_path, body, captcha_action, method, timeout, meta):
        ...
        # [VÁ RIÊNG 18/08] url_path có thể đã mang sẵn query (vd
        # `?clientContext.tool=PINHOLE`) — nối "?key=" lần nữa làm hỏng giá trị
        # tham số cuối ("PINHOLE?key=AIza..." → Google trả 400 INVALID_ARGUMENT).
        _sep = "&" if "?" in url_path else "?"
        url = f"{API_BASE}{url_path}{_sep}key={API_KEY}"
    ...
    async def ui_generate(self, params: dict, timeout=None):
        ...
        if isinstance(res, dict):
            mid, url = res.get("mediaId"), res.get("videoUrl")
            if mid and url:
                self.media_urls[str(mid).lower()] = url
            if mid and res.get("videoBase64"):
                self.media_blobs[str(mid).lower()] = res["videoBase64"]
            if isinstance(res.get("credits"), (int, float)):
